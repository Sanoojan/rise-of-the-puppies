...
LSKnet adapted from: https://github.com/zcablii/Large-Selective-Kernel-Network
...
__all__ = [
    "LSKNet",
    "build_lsknet_backbone",
    "build_lsknet_fpn_backbone"
]


## Inits taken from: https://github.com/open-mmlab/mmengine/blob/a5f48f7d99ae250b329272f88b961af1d3ebcf1e/mmengine/model/weight_init.py
...
class Block(nn.Module):
    def __init__(self, dim, mlp_ratio=4., drop=0.,drop_path=0., act_layer=nn.GELU, norm_layer=None):
        super().__init__()
        if norm_layer:
...
class OverlapPatchEmbed(nn.Module):
    ...
    def __init__(self, patch_size=7, stride=4, in_chans=3, embed_dim=768, norm_layer=None):
        super().__init__()
    ...
    def __init__(self, in_chans=3, embed_dims=[64, 128, 256, 512],
        ...
        for i in range(num_stages):
            ...
            patch_embed = OverlapPatchEmbed(patch_size=7 if i == 0 else 3,
                                            ...
                                            embed_dim=embed_dims[i], norm_layer=norm_layer_block)

            block = nn.ModuleList([Block(
                dim=embed_dims[i], mlp_ratio=mlp_ratios[i], drop=drop_rate, drop_path=dpr[cur + j],norm_layer=norm_layer_block)
